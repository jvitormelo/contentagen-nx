...
export function ReviewStep({ form }: { form: AgentForm }) {
  ...
  const singleColumnItems = [
    ...
    },
    {
      ...
      value: formatValue(String(form.getFieldValue("brand.integrationStyle") ?? "")),
    },
    {
      icon: <GlobeIcon className="w-4 h-4" />,
      label: "Language",
      value: formatValue(String(form.getFieldValue("language.primary") ?? "")),
    },
    {
      icon: <PaintbrushIcon className="w-4 h-4" />,
      label: "Formatting Style",
      value: formatValue(String(form.getFieldValue("formatting.style") ?? "")),
    },
  ];
  ...
  const communicationStyle = {
    ...
    value: formatValue(String(form.getFieldValue("voice.communication") ?? "")),
  };
